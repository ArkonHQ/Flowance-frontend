...
interface InvoicesBulkActionsProps {
  ...
  onBulkStatusChange: (newStatus: Invoice['status']) => void | Promise<void>
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-lg animate-in fade-in slide-in-from-bottom-5 duration-300">
      <div className="bg-card/90 dark:bg-card/85 backdrop-blur-xl border border-border/40 px-4 py-3 rounded-2xl shadow-2xl flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Select onValueChange={(val) => onBulkStatusChange(val as Invoice['status'])}>
            <SelectTrigger className="h-9 px-3 bg-background border-border/50 hover:bg-muted text-xs font-bold text-foreground w-32.5 rounded-xl">
            ...
            <DialogContent className="sm:max-w-md bg-card/80 backdrop-blur-xl border border-border/40 shadow-2xl p-0 gap-0 overflow-hidden rounded-xl">
              ...
              <div className="p-6 space-y-5">
                <DialogHeader className="space-y-4 text-center">
                  ...
                  <DialogTitle className="text-lg font-bold text-foreground">Delete {selectedCount} Invoices?</DialogTitle>
                  <DialogDescription className="text-xs text-muted-foreground leading-relaxed">
                    Are you sure you want to permanently delete the {selectedCount} selected invoices? This action cannot be undone.
                  </DialogDescription>
                </DialogHeader>
